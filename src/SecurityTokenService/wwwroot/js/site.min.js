...
</li>`;$("#"+n).append(i)}function initDiagnostics(){$.ajax({xhrFields:{withCredentials:!0},type:"GET",url:"diagnostics",success:function(n){if(n.claims&&n.claims.length>0){const t=$("#claims");for(let i=0;i<n.claims.length;++i){const r=n.claims[i];t.append(`<dt>${r.type}</dt>`);t.append(`<dd>${r.value}</dd>`)}}if(n.properties&&n.properties.length>0){const t=$("#properties");for(let i=0;i<n.properties.length;++i){const r=n.properties[i];t.append(`<dt>${r.key}</dt>`);t.append(`<dd>${r.value}</dd>`)}}},error:function(n){n.status===401&&(window.location.href="login.html?returnUrl=/diagnostics.html")}})}function initLoggedOut(){const n=getQueryParam("postLogoutRedirectUri"),i=getQueryParam("clientName"),t=getQueryParam("signOutIframeUrl"),r=getQueryParam("automaticRedirectAfterSignOut");n&&($("#clientName").text(i),$("#postLogoutRedirect").show(),r&&(window.location=n));t?($("#signOutIframe").show(),$("#signOutIframeUrl").attr("src",t)):window.location="/"}function initLogin(){$.validator.setDefaults({errorContainer:"div.error",errorLabelContainer:$("#form div.error"),wrapper:"label"});const n=$("#message");$("#sendSmsBtn").click(function(){n.hide();const t=$("#phoneNumber").val();!t||t.length>24||t.length<11?(n.text("手机号不正确"),n.show()):$.ajax({xhrFields:{withCredentials:!0},type:"POST",url:"account/sms",dataType:"json",contentType:"application/json",data:JSON.stringify({phoneNumber:t,countryCode:"+86"}),success:function(t){if(t.code!==200){if(t.message){const i=t.message.split("\n");let r="";for(let n=0;n<i.length;++n){const t=i[n];t&&(r+=n==0?t:"<br/>"+t)}n.html(r)}else printError(t.code);n.show()}else $("#sendSmsBtn").popover()},error:function(){n.show();n.text("服务器出小差")}})});$("#loginButton").click(function(){const n=$("#message");n.hide();$("#form").validate({rules:{username:{required:!0},password:{required:!0},captchaCode:{required:!0}},messages:{username:{required:"用户名不能为空"},password:{required:"请输入密码"},captchaCode:{required:"请输入验证码"}},submitHandler:function(t){let i=getQueryParam("returnUrl");i=i?i:"";const u=t.data,r=$(t).serialize()+"&returnUrl="+encodeURIComponent(i);$.ajax({xhrFields:{withCredentials:!0},type:"POST",url:"account/login",data:r,success:function(t){if(t.location){const i=t.location;let n=window;while(n!==n.top)n=n.top;n.location.href=i}else if(t.code!==200){if(t.message){const i=t.message.split("\n");let r="";for(let n=0;n<i.length;++n){const t=i[n];t&&(r+=n==0?t:"<br/>"+t)}n.html(r)}else printError(t.code);n.show()}else window.location.href="/"},error:function(){n.text("服务器出小差");n.show()}})}})});$("#smsLoginBtn").click(function(){n.hide();$("#smsForm").validate({rules:{phoneNumber:{required:!0},verifyCode:{required:!0}},messages:{phoneNumber:{required:"手机号不能为空"},verifyCode:{required:"请输入验证码"}},submitHandler:function(t){let i=getQueryParam("returnUrl");i=i?i:"";const r=$(t).serialize()+"&returnUrl="+encodeURIComponent(i);$.ajax({xhrFields:{withCredentials:!0},type:"POST",url:"account/loginBySms",data:r,success:function(t){if(t.location){const i=t.location;let n=window;while(n!==n.top)n=n.top;n.location.href=i}else if(t.code!==200){if(t.message){const i=t.message.split("\n");let r="";for(let n=0;n<i.length;++n){const t=i[n];t&&(r+=n==0?t:"<br/>"+t)}n.html(r)}else printError(t.code);n.show()}else window.location.href="/"},error:function(){n.text("服务器出小差");n.show()}})}})});const t=document.getElementById("captcha-img");t.addEventListener("click",refreshCaptcha);refreshCaptcha()}function refreshCaptcha(){const t=document.getElementById("captcha-img");t.style.opacity=0;const i=`/api/v1.0/captcha/generate?_t=${Date.now()}`,n=new XMLHttpRequest;n.open("GET",i,!0);n.responseType="blob";n.onload=function(){if(n.status===200){const i=n.response;t.src=URL.createObjectURL(i);t.style.opacity=1}else t.alt="加载失败， 点击重试",console.error(n.status)};n.onerror=function(){t.alt="网络错误， 点击重试";console.error()};n.send()}function initChangePassword(){$.validator.setDefaults({errorContainer:"div.error",errorLabelContainer:$("#form div.error"),wrapper:"label"});$("#submitButton").click(function(){const n=$("#message");n.hide();$("#form").validate({rules:{username:{required:!0},newPassword:{required:!0,regex:/^(?=.*\d)(?=.*[A-Za-z])(?=.*[^\da-zA-Z\s]).{8,30}$/},oldPassword:{required:!0},confirmNewPassword:{required:!0}},messages:{username:{required:"用户名不能为空"},newPassword:{required:"请输入密码",regex:"密码必须包含字母、数字和特殊符号，最少 8 位"}},submitHandler:function(t){const i=$(t).serialize();$.ajax({xhrFields:{withCredentials:!0},type:"POST",url:"account/resetPassword2",data:i,success:function(t){if(t.code!==200)if(n.show(),t.message){const i=t.message.split("\n");let r="";for(let n=0;n<i.length;++n){const t=i[n];t&&(r+=n===0?t:"<br/>"+t)}n.html(r)}else printError(t.code);else window.location.href="/"},error:function(){n.show();n.text("服务器出小差")}})}})})}function initLogout(){const n=getQueryParam("logoutId");$("#logoutId").val(n)}function initSession(){$.get("session",n=>{if(n.data&&n.data.some(n=>n.type==="sub")){const i=$("#user");i.attr("sub",n.data.sub);let t=n.data.filter(n=>n.type==="name"),r;t.length===0&&(t=n.data.filter(n=>n.type==="email"));r=t[0].value;i.prepend(r);$("#userNav").show()}})}function getQueryParam(n){const t=new URLSearchParams(window.location.search);return t.get(n)}$(document).ready(()=>{const n=location.href;n.indexOf("login.html")>=0?initLogin():n.indexOf("logout.html")>=0?initLogout():n.indexOf("loggedout.html")>=0?initLoggedOut():n.indexOf("diagnostics.html")>=0?(initSession(),initDiagnostics()):n.indexOf("consent.html")>=0?(initSession(),initConsent()):n.indexOf("redirect.html")>=0?initRedirect():n.indexOf("changePassword.html")>=0?initChangePassword():n.indexOf("error.html")>=0?(initSession(),initError()):initSession()});